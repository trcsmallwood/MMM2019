...
#Extract participant colours
participant_cols <- participants_df$PreferredColour

#Set line and shape types for "random" bracket to be different from participants (dotted vs solid line, empty vs filled circle)
...
ggplot(round_correct_plot_df, aes(x = Round, y = PropCorrect, fill = Participant)) +
  ...
  theme_bw() + #Simple balck and white base theme
  guides(fill = guide_legend(nrow = 3, byrow = T)) + #Control number of rows in lengend
  theme(axis.ticks.length = unit(-0.2, "cm"), #Ticks marks inside
